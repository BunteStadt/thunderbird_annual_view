...
a fresh checkout. The container image prefers a Thunderbird 153-compatible install from
Mozilla's package repository, and it can also consume a direct
`THUNDERBIRD_DEB_URL` override for a newer `.deb` package if the distro
version is too old. The companion `gui` service exposes a browser-based
