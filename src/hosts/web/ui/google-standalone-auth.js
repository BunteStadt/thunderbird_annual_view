...
export function setupGoogleStandaloneAuth({ mount, getProvider, refreshCalendars, onAuthChange }) {
    if (!mount) {
        return { update: () => { }, signOutIfConnected: async () => { } };
    }
    ...
    const update = () => {
        ...
        status.textContent = authError
            || (!configured
                ? "Set your Google OAuth client ID in src/hosts/web/google-client-id.js."
    ...
    connectButton.addEventListener("click", async () => {
        ...
        try {
            ...
            await onAuthChange?.(provider.getAuthState().authenticated);
    ...
    return {
        update,
        async signOutIfConnected() {
            if (provider.getAuthState().authenticated) {
                await provider.signOut();
                await onAuthChange?.(false);
            }
        }
    };
}
