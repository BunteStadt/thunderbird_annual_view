...
async function loadStoragePortModule() {
    const modulePath = path.resolve(__dirname, '../../src/core/storage-port.js');
    return import(`file://${modulePath.replace(/\\/g, '/')}`);
}

async function loadIntegrationModule() {
    const modulePath = path.resolve(__dirname, '../../src/core/ics-calendar-integration.js');
...
test('ICS calendar integration removeCalendar removes calendar and triggers refresh', async (t) => {
    ...
    const calService = await import(`file://${path.resolve(__dirname, '../../src/core/providers/calendar-service.js').replace(/\\/g, '/')}`);
