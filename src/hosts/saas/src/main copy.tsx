...
function SiteHeader({ navigate, path, session, theme, onThemeToggle }: { navigate: Navigate; path: string; session: Session | null; theme: "light" | "dark"; onThemeToggle: () => void }) {
    ...
    return (
        <header className="site-header">
            <nav className="page-width nav-shell" aria-label="Primary navigation">
                ...
                <button className="icon-button theme-toggle" type="button" onClick={onThemeToggle} aria-label={`Use ${theme === "dark" ? "light" : "dark"} theme`} aria-pressed={theme === "dark"} title={`Use ${theme === "dark" ? "light" : "dark"} theme`}>
...
function App() {
    ...
    const [theme, setTheme] = useState<"light" | "dark">(() => {
        const savedTheme = globalThis.localStorage?.getItem("year-view-landing-theme");
        return savedTheme === "dark" || savedTheme === "light" ? savedTheme : globalThis.matchMedia?.("(prefers-color-scheme: dark)").matches ? "dark" : "light";
    });
    ...
    useEffect(() => {
        globalThis.localStorage?.setItem("year-view-landing-theme", theme);
    }, [theme]);
    ...
    const shellClassName = isAuthenticatedApp ? "app-shell" : isEmbeddedDemo ? undefined : `${isLanding ? `landing-shell landing-theme-${theme}` : "saas-shell"} saas-theme-${theme}`;

    return <div className={shellClassName} data-theme={!isAuthenticatedApp && !isEmbeddedDemo ? theme : undefined}>
        {!isAuthenticatedApp && !isEmbeddedDemo && (!isAppRoute || authReady) && <SiteHeader navigate={navigate} path={path} session={session} theme={theme} onThemeToggle={() => setTheme((value) => value === "dark" ? "light" : "dark")} />}
        {page}
